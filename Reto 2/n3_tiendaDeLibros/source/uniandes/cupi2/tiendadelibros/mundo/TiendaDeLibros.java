...
public class TiendaDeLibros
{
    ...
    public int darCantidadTransaccionesAbastecimiento( String pIsbn )
    {
	 ...
	 /**
	  * Método para la extensión 1. RN1 - Aplicar descuentos
	  * @return Respuesta 1. 
	  */
	 public String metodo1( int[] lim_inf , int[] lim_sup , double[] descuentos )
	 {	
	 	double precioActual = 0;
	 	boolean precioDescontado;
	 	
	 	// Revisa que el catálogo no esté creado
	     if ( catalogo.size( ) > 0 ) {
	     	
	     	// accedemos libro a libro para cada dentro del catalogo    	
	     	for ( Libro libro : catalogo) {        		
	         	//  Valor de comparacion 
	         	precioActual = libro.darPrecioVenta();
	         	precioDescontado = libro.darPrecioDescontado();
	         		
	         	if ( precioDescontado == false) {
	         			
	         		for ( int i = 0 ; i < lim_inf.length ; i++ ) {
	         			// límites inferiores descuento (inclusivo)
	         			// límites superiores descuento (no inclusivo)
	         			if ( precioActual >= lim_inf[i] &&  precioActual < lim_sup[i]) {
	         				// calcular el precio actualizado para el valor de venta
	         				double porcentajeDescuento = descuentos[i];
	         				libro.aplicarDescuento(porcentajeDescuento);
	         			}
	         		}
	         	}    		    		
	     	}        	
	 ...
	 /**
	  * Método para la extensión 2. RN2 -Dar transaccion de venta mas costosa
	  * @return Respuesta 2. Transacción de venta más costosa realizada
	  */
	 public Transaccion metodo2( )
	 {
	 	// Inicializar Valor de comparacion 
	     double mejorVenta = 0;
	     // Variables de la transaccion
	     int cantidadTransaccion = 0;
	     Transaccion.Tipo tipoTransaccion;
	     // Transaccion de Salida Nula 
	     ...
	     if ( catalogo.size( ) > 0 ) {
	     	for ( Libro libro : catalogo ) {
	         	...
	         	//  Valor de comparacion 
	     		double precioUnitario = libro.darPrecioVenta();
	     		double valorTotal = 0;
	         	    		
	     		// Determinar el valor de la transaccion mas costosa
	 			// Transaccion ( tipoTransaccion, cantidadTransaccion, fechaTransaccion, idTransaccion )
	     		for ( Transaccion transaccion : libro.darTransacciones() ) {
	     			tipoTransaccion = transaccion.darTipo();
	     			if ( tipoTransaccion.equals(Transaccion.Tipo.VENTA) ) {        				
	     				cantidadTransaccion = transaccion.darCantidad();     			
	         			valorTotal = cantidadTransaccion * precioUnitario;  
	     			}
	     			
	     			// Mejoro el valor de la transaccion
	     			// Transaccion( Tipo pTipo, int pCantidad, String pFecha, String pIdTransaccion )
	         		if ( valorTotal > mejorVenta) {            			            			
	         			mejorVenta = valorTotal;                		
	             		mejorTransaccion = transaccion;
	         		}    		            		
	     		}    		 
	         }
	     	// Salida indica la transaccion que representa el mayor movimiento
