...
public class SimuladorBancario
{
    ...
    {    	
    	mesActual += 1;
    	inversion.actualizarInteresesPorPasoMes(mesActual);
    	ahorros.actualizarInteresPorPasoMes();
    ...
    {
    	// Avanza la simulación el número de meses indicado por el usuario.           
    	for (int i = 1 ; i <= pMeses; i=i+1) {
    		// nuevoSaldoCuentaAhorros, nuevoSaldoCDT
    		avanzarMesSimulacion(); // actualizar();
    	}  
    }
    ...
    {	
        // Informa al usuario del interés total generado e inicia una nueva simulación.    	
    	double interesCDTGenerado = inversion.darInteresesGenerados();
    	double interesAhorrosGenerado = ahorros.darInteresesGenerados();
    	double interesTotalGenerado = interesCDTGenerado + interesAhorrosGenerado;
    	// Actualizar valores por defecto nueva simulación
    	mesActual = 1;
    	ahorros.restablecer();
    	corriente.restablecer();
    	inversion.restablecer();
         	
    	return interesTotalGenerado;
    }
}